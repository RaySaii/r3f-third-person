 ...
 */
function getAngleBetweenVectors(v1: THREE.Vector3, v2: THREE.Vector3, dotThreshold = 0.0005) {
  let angle
  const dot = v1.dot(v2)
...
function getRotationDirection({left = false, right = false, isMouseLooking = false}) {
  let direction = 0
  if (!isMouseLooking) {
    if (left) {
      direction = -1
...
function getMovementDirection({up, down, right, left, isMouseLooking} = {} as any) {
...
export default function getInputMovementRotation(inputs: any) {
